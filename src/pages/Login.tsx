...
import { signInWithEmailAndPassword } from 'firebase/auth'; // SDK oficial
import { auth } from '../services/firebase'; // La instancia de auth
import { useAuth } from '../context/AuthContext';
import { Printer, Lock, Mail, AlertCircle, Eye, EyeOff } from 'lucide-react';

export const Login: React.FC = () => {
  const navigate = useNavigate();
  const { refreshUsuario, usuario } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Refresca los datos del usuario desde Firestore (roles, etc.)
      await refreshUsuario();

      // 3. Redirección lógica basada en el estado del usuario cargado
      // Si el usuario requiere cambio de pass, se lo envía ahí, sino al dashboard
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden bg-slate-900">
      ...
      <div className="relative z-10 w-full max-w-md px-6">
        ...
        <p className="text-center mt-6 text-gray-400 text-xs tracking-widest uppercase">© 2026 Sistema de Gestión de Imprentas</p>
      </div>

      <style>{`
        @keyframes print { 0% { transform: translateY(-100%) rotate(-45deg); } 100% { transform: translateY(200%) rotate(-45deg); } }
